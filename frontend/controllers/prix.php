...
class Prix extends CI_Controller {
	...
	public function index($data = null)
	{
		...
		$data["div_title"] = "prix";		
		$data["districts"] = $this->district_model->getSimple();
		...
		$this->template->write_view('content', 'prix/vwPrix', $data);
	...
	public function find($nom_region,$date_deb,$date_fin){
		$nom_region = urldecode($nom_region);
		$where = array("date  >="=>date_debut($date_deb), "date <="=>date_fin($date_fin),"nom_region"=>$nom_region);
		$prix = $this->prix_model->getWhere($where);
		$this->output
		->set_content_type('application/json')
		->set_output(json_encode($prix));
	}
	public function get_actuel(){
		$date = date("Y-m-d");
	...
	public function save_androide(){
	  $district=$_POST['nom'];
	  $produit=$_POST['prenom'];
	  $prix_cons=$_POST['prix_cons'];
	  $prix_prod=$_POST['prix_prod'];
	  $date=$_POST['time_evm'];
	  
	  $data["code_district"] = $this->district_model->getId($district);
	  $data["produit_idproduit"] = $this->produit_model->getId($produit);
	  $data["date"] = $date;
	  $data["prix_conso"] = $prix_cons;
	  $data["prix_prod"] = $prix_prod;
	  
	  $array = array("date <="=>date_fin($data["date"]),"date >="=>date_debut($data["date"]),"prix.code_district"=>$data["code_district"],"produit_idproduit"=>$data["produit_idproduit"]);
		if ($this->check($array)) $this->prix_model->updateWhere($data,$array);
		else $this->prix_model->add($data);
	  	$array["success"] = 1;
		$this->output
