...
class Graph extends CI_Controller {
	...
	public function index($data = null){
		...
		$this->template->write('title',"Prix");
	...
	public function graph_price($data = null){
		$data["niveau1"] = array("text"=>"Graphique des prix");
		$data["districts"] = $this->districts->getPage();
		$data["regions"] = $this->regions->getPage();
		$this->template->add_js("assets/backend/js/sites/graph/graph_prix.js?".mktime());
		$this->template->add_css("assets/backend/js/lib/ui/css/jquery-ui-1.10.0.custom.css");
		$this->template->add_js("assets/backend/js/lib/highcharts/js/highcharts.js");
		$this->template->add_js("assets/backend/js/lib/highcharts/js/modules/exporting.js");
		$this->template->add_js("assets/backend/js/lib/ui/js/jquery-ui-1.10.0.custom.min.js");
		$this->template->add_js("assets/backend/js/lib/ui/js/jquery.ui.datepicker-fr.js");
		$this->template->write('title', "Graphique des prix");
		$this->template->write_view('header', 'partials/header', $data);
		$this->template->write_view('content', 'graph/vwGraph_price', $data);
		$this->template->render();
	}
	public function map(){
		$data["niveau1"] = array("text"=>"Découvrir les prix sur google map");
		$data["districts"] = $this->districts->getPage();
		$data["all_districts"] = $this->districts->get();
		$data["regions"] = $this->regions->getPage();
		$this->template->add_js("assets/frontend/js/map.js?".mktime());
		$this->template->write('title', "Map");
		$this->template->write_view('header', 'partials/header', $data);
		$this->template->write_view('content', 'graph/vwMap', $data);
		$this->template->render();
	}
	public function get(){
		$results = array();
	...
	public function prix($prod = "riz"){
		...
		switch (strtoupper($prod)){
			case "AUTRE" : 
				$where_prod = array("idproduit >="=>5,"idproduit <="=>8);
				$data["graph_title"] = "Prix des produits par district (Source: CSA)"; 
			break;
			case "RIZ" : 
				$where_prod = array("idproduit >="=>1,"idproduit <"=>5); 
				$data["graph_title"] = "Prix du riz par district (Source: OdR)";
			break;
		}
		//print_r($where_prod);die;
		$produits = $this->produit_model->getWhere($where_prod);
